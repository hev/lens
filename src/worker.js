...
export function searchBody(query, topK = 16) {
  return {
    rank_by: [IMAGE_FIELD, "ANN", ["Embed", query]],
    top_k: Math.max(1, Math.min(Number(topK) || 16, 30)),
    include_attributes: INCLUDE,
  };
}

export function servingContract() {
  return {
    prefer: SERVING,
    model: MODEL,
    dims: DIMS,
...
export async function proxySearch(request, env) {
  ...
  const namespace = env.LAYER_NAMESPACE || "lens-commons-quality";
  ...
  if (!response.ok) {
    return json(
      { detail: upstream.detail || upstream.message || upstream.error || text.slice(0, 1000) },
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/config") {
      return json({
        namespace: env.LAYER_NAMESPACE || "lens-commons-quality",
        gateway: env.LAYER_GATEWAY_URL || "https://aws-us-east-1.hevlayer.com",
        serving: servingContract(),
      });
    }
    if (url.pathname === "/api/stats") return proxyStats(env);
